...
Supported public version: `0.1.x`; `0.2.0` remains pending until its release gate completes. No plugin build is trusted unless its source commit, target ATAK build, signer fingerprint, and SHA-256 are independently verified. SDK compile compatibility does not replace a separately authorized runtime installation test.
