...
public class HonkCotCodecTest {
    ...
    @Test public void permanentLifetimeRoundTripsWithoutLocalExpiry() { long now = 1700000000000L; CameraObservation o = new CameraObservation("honktak-" + UUID.randomUUID(), 36, -95, CameraObservation.CameraClass.FIXED, 42, CameraObservation.Confidence.MEDIUM, CameraObservation.Status.ACTIVE, "", now, CameraObservation.PERMANENT); CameraObservation p = HonkCotCodec.parse(HonkCotCodec.serialize(o), now); assertTrue(p.isPermanent()); assertEquals(CameraObservation.PERMANENT, p.staleAtMs); }
    @Test public void persistedPermanentSurvivesTransportStaleForReload() { long now = 1700000000000L; CameraObservation o = new CameraObservation("honktak-" + UUID.randomUUID(), 36, -95, CameraObservation.CameraClass.FIXED, 42, CameraObservation.Confidence.MEDIUM, CameraObservation.Status.ACTIVE, "", now, CameraObservation.PERMANENT); CameraObservation p = HonkCotCodec.parsePersisted(HonkCotCodec.serialize(o), now + 8L * 24 * 60 * 60 * 1000); assertTrue(p.isPermanent()); }
